...
def main(args):
    ...
    dataset = PathImageFolder(args.data_path, transform=transform)
    ...
    for x, y, paths in tqdm(loader):
        ...
        idx = train_steps
        for i, (features, label, path) in enumerate(zip(x, y, paths)):
            np.save(f"{args.features_path}/imagenet256_features/{idx+i}.npy", features)
            np.save(f"{args.features_path}/imagenet256_labels/{idx+i}.npy", label)
            np.save(f"{args.features_path}/imagenet256_path/{idx+i}.npy", path)
