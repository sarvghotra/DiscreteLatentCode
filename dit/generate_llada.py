...
def generate_text(
    ...
    Args:
        ...
        mask_id: The toke id of [MASK] is 126336.
    """
    x = torch.full((1, sems.shape[1] + gen_length), mask_id, dtype=torch.long).to(
        model.device
    )
    x[:, -sems.shape[1] :] = sems.clone()
    # x = prompt.clone()
    ...
    for num_block in range(num_blocks):
        block_mask_index = (
            x[
                :,
                -(sems.shape[1] + (num_block + 1) * block_length) : -(
                    sems.shape[1] + num_block * block_length
                ) :,
                # num_block * block_length :
                # + (num_block + 1) * block_length :,
            ]
        ...
        for i in range(steps):
            ...
            x0_p[:, : -(sems.shape[1] + (num_block + 1) * block_length)] = -np.inf
            # x0_p[:, (num_block + 1) * block_length :] = -np.inf
