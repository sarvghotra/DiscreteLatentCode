...
def main(script_args, training_args, model_args):
    ...
    train_dataset = load_dataset(
        "webdataset",
        # data_files={"train": "/home/j/jeet/links/scratch/data/blip3-kale/blip3-kale_imgs/*.tar"},
        data_files={"train": "/home/j/jeet/links/scratch/data/blip3-kale/tmp/*.tar"},
        split="train",
        streaming=True,
    )

    def extract_and_rename_field(example):
        cogvlm_caption = example['json']['cogvlm_caption']
        example['txt'] = cogvlm_caption
        return example

    def add_prompt(example):
        example['text'] = "Encode the following caption describing the image in a sequence of special tokens. The caption is: " + example['text'] + " The sequence of special tokens: "
        return example
    ...
    # train_dataset = train_dataset.map(extract_and_rename_field)
