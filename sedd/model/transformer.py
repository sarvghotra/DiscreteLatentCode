...
class UnSharedEmbeddingLayer(nn.Module):
    def __init__(self, dim, L, V):
        super().__init__()
        scale = dim**-0.5
        self.L = L
        self.V = V
        embedding_table = scale * torch.randn(L, V, dim)
        self.embedding_table = nn.Parameter(embedding_table)

    def forward(self, x):
        o = torch.nn.functional.one_hot(x, self.V).float()
...
class SEDD(nn.Module, PyTorchModelHubMixin):
    def __init__(self, config):
        ...
        if config.model.shared_embedding:
            self.vocab_embed = EmbeddingLayer(config.model.hidden_size, vocab_size)
        else:
            self.vocab_embed = UnSharedEmbeddingLayer(
                self.config.model.hidden_size, self.config.model.length, self.vocab_size
            )
        self.pos_embed = nn.Parameter(
            torch.empty((config.model.length, config.model.hidden_size))
        )
        torch.nn.init.kaiming_uniform_(self.pos_embed, a=math.sqrt(5))
